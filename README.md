...
- 🔗 智能句子对齐：采用 Needleman-Wunsch 风格算法，处理插入、删除和替换（已使用 numpy 优化内存）
...
- 🖥️ 图形化界面（GUI）支持（基于 customtkinter），提供实时进度条和日志显示
- 🎛️ 高级参数可配置：通过配置文件调整对齐惩罚、相似度偏移、默认时长、束搜索宽度、VAD（语音活动检测）等，适应不同场景
- 📊 实时进度反馈：GUI 和 CLI（对齐阶段）均显示进度条，操作更直观
...
**注意**：CLI 默认会显示 Stable‑Whisper 的默认进度条；如需完全静默（例如在脚本中使用），可在调用 `direct_it` 时设置 `verbose=False`（当前版本已支持）。
...
- `max_combine`：限制一句台本最多合并的 Whisper 片段数。
...
## 项目结构
- `director.py`：核心模块，包含语音识别、句子对齐（numpy 优化）、时间戳映射、字幕保存等功能。
...
- `subtitles_toolkit.py`：字幕解析与时间轴插值工具函数。
