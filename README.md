...
- 🎛️ 高级参数可配置：通过配置文件调整对齐惩罚、相似度偏移、默认时长等，适应不同场景
...
```
支持修改的键（包括高级参数）：`model`, `lang`, `device`, `compute`, `gap_penalty`, `similarity_offset`, `default_duration`。
...
```
界面包含以下核心控件：
- **模型路径**：选择本地 Faster Whisper 模型文件夹
- **语言代码**：下拉选择音频语言（支持日语、中文、英文等）
- **设备类型与计算类型**：根据硬件选择
- **音频文件**、**台本文件**：通过浏览按钮选择
- **输出名称**（可选）、**输出格式**（SRT/LRC）
- **预处理台本**：勾选后自动清理空行和方括号标识
- **开始处理**：点击后实时显示日志，处理完成弹出提示
