...
- **视频输入**：支持 MP4、MKV、AVI、MOV 等常见视频格式。处理视频文件时需要 **ffmpeg**，程序按以下顺序查找：项目 `ffmpeg/` 目录 → 系统 PATH。如未找到会提示安装。可将 ffmpeg 放入项目根目录的 `ffmpeg/` 文件夹下（详见 `ffmpeg/README.md`）。
- 台本文件需为 UTF-8 编码的纯文本。
